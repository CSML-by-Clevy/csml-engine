...
pub fn execute_step(flow: &Flow, name: &str, mut data: Data, index: Option<i64>) -> Result<MessageData, ErrorInfo> {
    match (search_for(flow, name), index) {
        ...
        (Some(Expr::Scope { scope, block_type, ..}), None) => {
