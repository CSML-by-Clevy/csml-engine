...
pub fn url(args: HashMap<String, Literal>, interval: Interval) -> Result<Literal, ErrorInfo> {
    ...
    match &search_or_default(&args, "url") {
        Some(href) if href.primitive.get_type() == PrimitiveType::PrimitiveString => {
            url.insert("url".to_owned(), href.clone());

            match search_or_default(&args, "title") {
                Some(title) => url.insert("title".to_owned(), title),
                None => url.insert("title".to_owned(), href.clone()),
            };

            match search_or_default(&args, "text") {
                Some(title) => url.insert("title".to_owned(), title),
                None => url.insert("title".to_owned(), href.clone()),
            };
