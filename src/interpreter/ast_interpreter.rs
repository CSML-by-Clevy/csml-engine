...
fn match_obj(lit1: &Literal, lit2: &Literal) -> Result<Literal, ErrorInfo> {
    ...
    match (expr1, expr2) {
        (exp1, ..) if check_if_not_operator(infix) && check_if_ident(exp1) => {
            ...
            match get_var_from_ident(exp1, data) {
                ...
                Ok(SmartLiteral { literal, interval }) => Ok(SmartLiteral {
                    literal: Literal::BoolLiteral(false),
                    interval,
                }),
                Err(..) => Ok(SmartLiteral {
        ...
        (exp1, exp2) if check_if_ident(exp1) && check_if_ident(exp2) => cmp_lit(
            infix,
...
fn match_builtin(object: Literal) -> Result<Literal, ErrorInfo> {
    match object {
        Literal::ObjectLiteral {
            ref name,
            ref value,
        } if name == TYPING => typing(value),
        Literal::ObjectLiteral {
            ref name,
            ref value,
        } if name == WAIT => wait(value),
        Literal::ObjectLiteral {
            ref name,
            ref value,
        ...
        Literal::ObjectLiteral {
            ref name,
            ref value,
        } if name == ONE_OF => one_of(value),
        Literal::ObjectLiteral {
            ref name,
            ref value,
        } if name == QUESTION => question(value, name.to_owned()),
        Literal::ObjectLiteral {
            ref name,
            ref value,
        } if name == BUTTON => button(value),
        Literal::ObjectLiteral {
            ref name,
            ref value,
...
fn expr_to_literal(expr: &Expr, data: &mut Data) -> Result<Literal, ErrorInfo> {
    match expr {
...
fn match_functions(action: &Expr, data: &mut Data) -> Result<MessageType, ErrorInfo> {
    match action {
        Expr::FunctionExpr(ReservedFunction::As(name, expr)) => {
            ...
            match msg {
                MessageType::Msg(Message { ref content, .. }) => {
                    data.step_vars.insert(name.to_owned(), content.clone());
                }
                MessageType::Empty => {}
            };
            Ok(msg)
        ...
        Expr::IdentExpr(ident, ..) => match get_var(ident, data) {
            Ok(val) => Ok(MessageType::Msg(Message::new(val))),
            Err(_e) => Ok(MessageType::Msg(Message::new(Literal::StringLiteral(
                "NULL".to_owned(),
            )))),
        },
        Expr::LitExpr { .. } => Ok(MessageType::Msg(Message::new(expr_to_literal(
            action, data,
        )?))),
        Expr::VecExpr(..) => Ok(MessageType::Msg(Message::new(expr_to_literal(
            action, data,
        )?))),
            ...
            (
                ...
                if let Some(SmartIdent { ident, interval }) = opt {
                    root = root.add_to_memory(
                        ident.to_owned(),
                        gen_literal_form_event(data.event, interval)?,
                    );
                }
                return Ok(root + interpret_block(args, data)?);
            }
