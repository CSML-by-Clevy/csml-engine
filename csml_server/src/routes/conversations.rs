...
pub async fn get_open(body: web::Json<Client>, req: actix_web::HttpRequest) -> HttpResponse {
    ...
    let res = thread::spawn(move || get_open_conversation(&body))
        .join()
        .unwrap();
...
pub async fn close_user_conversations(
    body: web::Json<Client>,
    ...
    let res = thread::spawn(move || user_close_all_conversations(body.clone()))
        .join()
        .unwrap();
...
pub async fn get_client_conversations(
    query: web::Query<GetClientInfoQuery>,
    req: actix_web::HttpRequest,
) -> HttpResponse {
    if let Some(value) = validate_api_key(&req) {
        eprintln!("AuthError: {:?}", value);
...
mod tests {
    ...
    async fn test_get_open_conversations() {
        let mut app = test::init_service(App::new().service(get_open)).await;
    ...
    async fn test_close_conversations() {
        let mut app = test::init_service(App::new().service(close_user_conversations)).await;
    ...
    async fn test_get_conversations() {
        let mut app = test::init_service(App::new().service(get_client_conversations)).await;

        let (user_id, channel_id, bot_id) = ("test", "conversations-channel", "botid");

        let resp = test::TestRequest::get()
            .uri(&format!(
                "/conversations?user_id={}&channel_id={}&bot_id={}",
